...
//	private float AddSpeed =0f;
	void Update () {
		...
		transform.position = Vector3.Lerp (transform.position, cameraTarget, Time.deltaTime* CameraMovement);


		//transform.LookAt (target.position);
		Quaternion rotation = Quaternion.LookRotation(new Vector3( target.position.x , target.position.y - zLookAdd, target.position.z) - transform.position);
