...
[RequireComponent(typeof(SwitchWeapons))]
public class PlayerControl : MonoBehaviour {
    ...
    private int HashSecondAttackSecuirty;
    private int HashDizzy;
    #endregion

    #region Variables

    //audio listener
    public GameObject AudioListenerToSpawn;
...
//		public float AttackDuration;
        ...
        public Joystick JoystickScriptReference;
        public GameObject JoystickCanvasPrefab;
...
//		#endregion
    ...
    private void Start(){
        ...
        //Audio listener
        Instantiate(AudioListenerToSpawn);
        //turn off dizzy effect
        if (DizzyOBJ != null)
            DizzyOBJ.SetActive(false);

        Instantiate(spawnParticles, transform.position, Quaternion.identity * Quaternion.Euler(new Vector3(90.0f, 0.0f, 0.0f)));
        //Making sure the buttons work
        if (GameObject.FindGameObjectWithTag("CanvasHUD") != null)
        {
            Button EvadeButtonSearch = GameObject.Find("EvadeButton").GetComponent<Button>();
            EvadeButtonSearch.onClick.AddListener(() => EvadeButton());
            Button AttackButtonSearch = GameObject.Find("Button:Attack").GetComponent<Button>();
            AttackButtonSearch.onClick.AddListener(() => AttackButton());
        }
        else
        {
            Debug.LogError("MARJOR ERROR: Canvas Pause-Menu required. Ref: PlayerControl");
        }
        ...
        if (JoystickScriptReference == null && MyInput==InputSwitch.Android)
        {
            FixNullVariables();
        }
...
//		agent = GetComponent<NavMeshAgent> ();
		...
		//animator
		HashRunning = Animator.StringToHash ("Traveling");
        ...
        HashSecondAttackSecuirty = Animator.StringToHash("SecondAttackSecurity");
        HashDizzy = Animator.StringToHash("Dizzy");
...
#region Movement
			...
			if(MyInput==InputSwitch.Android){
                if (JoystickScriptReference == null)
                    FixNullVariables();
				else if (!JoystickScriptReference.gameObject.activeSelf)
					JoystickScriptReference.gameObject.SetActive(true);
                //The commented code below is supposed to work, however I could not get it to work succsesfully. So I'm doing it manually
                Vector3 ManualData = JoystickScriptReference.GetTouchInputManual();
                input = new Vector3(ManualData.x, 0, ManualData.y);//new Vector3 (CrossPlatformInputManager.GetAxis ("Horizontal"), 0, CrossPlatformInputManager.GetAxis ("Vertical"));
            }
			else
			...
			if(MyInput==InputSwitch.Computer){
				...
				if(JoystickScriptReference!=null){
                    GameObject search = GameObject.Find("MobileSingleStickControl");
                    if(search!=null)
...
//			}
		    ...
		    && MyInput!=InputSwitch.Android) {
                ...
                //Debug.Log("Hash running"+HashRunning);
				PlayerAnimator.SetBool(HashRunning,false);
                //Debug.Log("Launch 2");
...
#region DOT
        ...
        {
            float additiveSuppresion=10f;
            float multiply = 1 + (additiveSuppresion - (currentHealth/maxHealth)*additiveSuppresion);
            healthSlider.value = Mathf.Lerp(healthSlider.value, currentHealth / maxHealth, Time.deltaTime * multiply);
        }
	}

    private void FixNullVariables()
    {
        bool RIEmpty = false;
        foreach(Animator current in RunIcons)
        {
        ...
        if(RIEmpty)
        {
            GameObject[] IconOBJS= GameObject.FindGameObjectsWithTag("HUDEvadeIcon");
            Animator IC1 = IconOBJS[0].GetComponent<Animator>();
            Animator IC2 = IconOBJS[1].GetComponent<Animator>();
            Animator IC3 = IconOBJS[2].GetComponent<Animator>();
            if (IC1 != null && IC2 != null && IC3 != null)
                RunIcons = new Animator[] { IC1, IC2, IC3 };
            else
                Debug.LogWarning("Missing Run Icons!");
        }
        if (JoystickScriptReference == null) {

            if (MyInput == InputSwitch.Android) {
                GameObject searcha = GameObject.Find("MobileSingleStickControl");
                if (searcha != null)
                    searcha.SetActive(true);
            }
            GameObject search = null;
            search = GameObject.Find("MobileJoystick");
            //Debug.Log("Search= "+search);
            if(search==null)
                Instantiate(JoystickCanvasPrefab, Vector3.zero, Quaternion.identity);
            search = GameObject.Find("MobileJoystick");
            JoystickScriptReference = search.GetComponent<Joystick>();
        }
    }
	...
	IEnumerator AttackCoRout ()
	{
        ...
        /*float prevtimeTurn = Time.time;
        float WaitTimeTimer=WaitTime;
        while (WaitTimeTimer > WaitTime*3/5)
        {
            //Aiming the attack box
            if (secondInput != Vector3.zero)
            {
	...
	//			Debug.LogError("ATTACK hit point");
		...
		if (CurrentWeapon.Name == "Sword" && MyState==State.Action) {
			...
			float timer = (1 - CurrentWeapon.StrikeTime) * CurrentWeapon.AttackDuration;
			...
			while (timer>0 && !SecondAttack) {
				...
				if (Input.GetAxis ("Attack") == 1){
					SecondAttack = true;
                    PlayerAnimator.SetTrigger(HashSecondAttack);//Needs to be here. as the animator will go to idle before second attack begins
                    PlayerAnimator.SetBool(HashSecondAttackSecuirty,true);
                    break;
				}
        ...
        //			StartCoroutine (AttackCoRout ());
        //		} else {
        PlayerAnimator.SetBool(HashSecondAttackSecuirty, false);
        if (MyState != State.Dead && MyState !=State.Knockback) {
	...
	{
        PlayerAnimator.SetBool(HashDizzy, true);
		...
		if (MyState != State.Dead) {
            if (DizzyOBJ != null)
                DizzyOBJ.SetActive(false);
            PlayerAnimator.SetBool(HashDizzy, false);
            MyState = State.Moving;
...
#region Floating through the air
		...
		while (timer>0) {
            ...
            //Visual dust kick during movement
            			GameObject CurrentDustKick = (GameObject)Instantiate (DustKick, transform.position, transform.rotation * Quaternion.Euler(new Vector3(0.0f, 90.0f, 0.0f)));
...
//			}
	...
	private void Knockback (float knockbackDuration, float knockbackDistance, Vector3 PointToTravelAwayFrom, bool interuptsAttack =true, bool ChainKB = false)
	{
		...
		if ((!KBIsActive || ChainKB) && CurrentInvincibilityDuration<=0.0f && MyState!=State.Dead && MyState != State.Evading) {
			StopCoroutine(KnockBackCoRout());
            //StopCoroutine(Evading());
            PlayerAnimator.SetBool(HashDizzy,false);
			//stop attack???
			if (interuptsAttack) {
				StopCoroutine (AttackCoRout ());
